...
it('allows sms when only a purchased balance remains', function () {
    $merchant = allowanceMerchant();
    $usage = app(MerchantUsageService::class);
    ...
    $merchant->refresh();
    $merchant->sms_included_remaining = 0;
    $merchant->sms_purchased_balance = 12;
    $merchant->save();

    expect($usage->canSendSms($merchant))->toBeTrue();
});

it('debits local sms purchased balance after included units are exhausted', function () {
    $merchant = allowanceMerchant([
        'sms_purchased_balance' => 3,
    ]);
    $usage = app(MerchantUsageService::class);
    ...
    $merchant->refresh();
    $merchant->sms_included_remaining = 0;
    $merchant->sms_purchased_balance = 3;
    $merchant->save();

    $usage->consumeSmsUnit($merchant, 'sms:test-debit-1');
    $merchant->refresh();

    expect((int) $merchant->sms_purchased_balance)->toBe(2);
});

it('includes local purchased balances in messaging totals', function () {
    $merchant = allowanceMerchant([
        'sms_purchased_balance' => 45,
        'whatsapp_purchased_balance' => 9,
        'ai_purchased_credits' => 18,
    ]);
    $usage = app(MerchantUsageService::class);
